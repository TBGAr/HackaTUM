...
class CracksDataset(utils.Dataset):
    ...
    def load_crack(self, dataset_dir, subset):
        ...
        #annotations = json.load(open(os.path.join(dataset_dir, "Annotations/merged_file.json")))
        ...
        # Add images
        for ann in glob.glob(dataset_dir+ "Annotations/*.json"):
            annotation = json.load(open(ann))
            annotation = [a for a in annotation if a['shapes']]
            print("Annotation")
            # Get the x, y coordinaets of points of the polygons that make up
            # the outline of each object instance. These are stores in the
            # shape_attributes (see json format above)
            # The if condition is needed to support VIA versions 1.x and 2.x.
            if type(annotation['shapes']) is dict:
                polygons = [r['points'] for r in annotation['shapes'].values()]
            else:
                polygons = [r['points'] for r in annotation['shapes']] 
